...
# 产物: out/aurora_udp_bridge_txdirect.bit + scripts/probes_txdirect.ltx
# =============================================================================
set proj  D:/FPGA/project_8
file mkdir $proj/out

open_project $proj/prj/project_8.xpr
set_property verilog_define {P8_TX_DIRECT} [get_filesets sources_1]
reset_run synth_1
...
write_checkpoint -force $proj/scripts/pre_impl_txdirect.dcp
close_design
open_checkpoint $proj/scripts/pre_impl_txdirect.dcp
if {[llength [get_debug_cores -quiet]] == 0} { error "debug cores lost after checkpoint reopen" }
implement_debug_core
write_debug_probes -force $proj/scripts/probes_txdirect.ltx
...
report_clocks            -file $proj/out/rpt_clocks_txdirect.rpt
report_timing_summary -delay_type min_max -report_unconstrained -check_timing_verbose \
                         -max_paths 10 -file $proj/out/rpt_timing_summary_txdirect.rpt
report_timing -max_paths 25 -sort_by slack -file $proj/out/rpt_timing_worst_txdirect.rpt
puts "TIMING: [get_property SLACK [get_timing_paths -max_paths 1 -sort_by slack]]"

# ---- 保存布线后检查点（位流出问题时免重跑实现）----
write_checkpoint -force $proj/scripts/post_route_txdirect.dcp
write_bitstream -force $proj/out/aurora_udp_bridge_txdirect.bit

puts "DBG_BUILD_DONE: bit=$proj/out/aurora_udp_bridge_txdirect.bit ltx=$proj/scripts/probes_txdirect.ltx"

# ---- 构建完清掉变体 define，避免污染后续正常构建（define 只影响综合）----
set_property verilog_define {} [get_filesets sources_1]
puts "TXDEFINE_CLEARED"
