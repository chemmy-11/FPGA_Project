...
- ✅ **会话 JSON 传输质量评测（09-21）**：质量档 100KB 100%+SHA 一致；性能档 10MB 99.84% @ 19.3 Mbps（PC 单流天花板）；容量档测出系统串行上限 ~23 Mbps（store-and-forward 流水线瓶颈，帧泵 v2 的靶子）。损坏/乱序全程 0。
- 🔍 帧泵 wr_full 二进制/格雷混比根因修复详见知识库《阶段二之九_prj9_调试记录_帧泵格雷满判_2026-09-21》；全量传输丢失定位证据链见《阶段二之九_prj9_测试报告_会话JSON全量传输质量评测_2026-09-20》。
- ⏭️ 下一步（prj9 尾巴）：B 侧探针位流锁死 Aurora 段 0.4% 丢失 → 帧泵 v2 多帧队列（冲 23Mbps 上限）→ 过载冻结根因 → IBERT 眼图 → 双板干线
