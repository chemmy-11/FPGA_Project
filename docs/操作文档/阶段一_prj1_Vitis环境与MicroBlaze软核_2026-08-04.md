---
type: 操作文档
摘要: 阶段一教学指南——Vitis 环境与 MicroBlaze 软核跑通 Hello World
阶段: 阶段一
...
- **首选**：与**前辈遗留工程**使用的 Vivado 版本一致（先问清楚或用 `vivado -version` 查遗留工程的版本文件）。版本不一致会导致工程升级提示，新手阶段少找麻烦。
...
- [ ] Vitis 环境安装完成，能建工程
- [ ] 已通读前辈遗留工程结构，能说出"它用了哪些 IP、数据流走向"
