...
if ($mode -eq 'quick') {
  ...
  $out = & $py scripts\json_storm.py testdata\session_full.jsonl --chunk 1360 --gap-ms 0.1 2>&1
  $out | Select-String -Pattern '收到|丢失|损坏' | ForEach-Object { $_.Line }
  $m = ($out | Select-String -Pattern 'JSON_STORM_SUMMARY').Line -replace '.*"lost":(\d+).*','$1'; if($m -match '^\d+
}
...
if(($segv | Where-Object { $_ -gt ($noise/2) }).Count -eq 0){ if($PcLost -eq 0){ Write-Host '>> 判决: 全链零丢失(各段差分为零) - 完美往返' -ForegroundColor Green } elseif($PcLost -gt 0){ Write-Host '>> 判决: 各段差分为零但 PC 有丢失 - 丢失在栈以前, 需复查' -ForegroundColor Yellow } else { Write-Host '>> 判决: 全链零丢失(各段差分为零)' -ForegroundColor Green } }){ $script:PcLost = [int]$m }
}
