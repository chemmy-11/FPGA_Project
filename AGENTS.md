...
- ✅ **会话 JSON 全量传输质量评测（09-20 晚）**：质量档 100%+SHA 一致 · 性能档 10MB 99.84% @19.3Mbps（PC 单流天花板）· 容量档系统串行上限 ~23Mbps；损坏/乱序全程 0
- 🔍 **prj9 尾巴 = 全量传输 0.4% 丢失定位**：已排除 PC 侧与泵A（ILA 计数器差分无辜），定位于 Aurora 渡纤段（随机相邻对、与节奏无关）→ 首要嫌疑 B 回显 pack 溢出；下一步 = B 侧探针位流（echo_b_ovf / unpack_b + Aurora CC/错误计数器入 ILA）锁死丢失点
- ⏭️ 帧泵 v2 多帧队列（冲 23Mbps 上限，消除单帧在途策略）→ 过载冻结根因（~26MB 累计或瞬时过载触发，重烧恢复）→ IBERT 眼图（空闲通道 C/D）→ 双板干线 → DDR/DMA 解冻
